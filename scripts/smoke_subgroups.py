...
def seed(path: Path, notes: Path) -> dict[str, str]:
    with Database(path) as library:
        ...
        empty = library.create_group("Empty", parent=root["id"])
        ids: dict[str, str] = {}
        for title in ("Loose", "Duplicate"):
            item, _ = library.add(ResolvedItem(title=title, url=f"https://example.test/{title}", source="smoke"))
            ids[title] = item["id"]
        item, _ = library.add(ResolvedItem(title="Alpha only", url="https://example.test/alpha", source="smoke"))
        ids["Alpha only"] = item["id"]
        library.connection.executemany(
            "INSERT INTO item_groups (group_id, item_id) VALUES (?, ?)",
            [(first["id"], ids["Duplicate"]), (second["id"], ids["Duplicate"]), (first["id"], ids["Alpha only"])],
        )
        notes.mkdir()
...
def run(binary: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="lit-tui-subgroups-") as directory:
        ...
        ids = seed(db, notes)
        env = os.environ.copy()
        for key in ("LIT_CONFIG", "LIT_DB", "LIT_VAULT", "LIT_NOTES_DIR", "LIT_TUI_DB", "LIT_TUI_NOTES_DIR", "LIT_TUI_VAULT"):
            env.pop(key, None)
        env.update({"LIT_TUI_BINARY": str(binary), "TERM": "xterm-256color", "XDG_CONFIG_HOME": str(root / "config"), "XDG_DATA_HOME": str(root / "data")})
        session = PtySession(["uv", "run", "lit", "--db", str(db), "--notes-dir", str(notes), "tui"], env)
        try:
            session.wait_until(lambda text: "Research / Alpha" in text and "Research / Beta" in text, "subgroup headers")
            screen = session.screen_text()
            if "General" in screen or "Research / Empty (0)" not in screen or "(empty)" not in screen or "Loose" not in screen:
            ...
            with Database(db) as library:
                if library.get(ids["Duplicate"])["status"] != "reading":
                    raise SmokeFailure(f"status action targeted the wrong subgroup occurrence: {session.screen_text()}")
            before = sorted(notes.glob("*.md"))
            session.send("\r")
            session.wait_until(lambda text: "EXISTING DUPLICATE NOTE" in text, "existing duplicate note")
            session.send("\x1b")
            time.sleep(0.3)
            session.send("\x7f")
            ...
            session.send("g")
            session.wait_until(lambda text: "Research / Alpha" in text and "Loose" in text, "root group view")
            root_view = session.screen_text()
            if "Research / Beta" not in root_view:
            ...
            session.send("q")
            session.wait_until(lambda _: session.process.poll() is not None, "clean quit", timeout=6)
